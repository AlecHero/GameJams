...
func scale_poly(from_points, to_points, weight):
	...
	for i in range(len(points)):
		...
		var pp_intersects : bool = len(get_world_2d().direct_space_state.intersect_point(pp, 1)) > 0
		if pp_intersects:
			if bigmode:
				new_points = points
				break
			else:
				# vvv is supposed to fix the collision situation where
				# you go bigmode in a 2 high area on a ledge, which makes
				# bigmode push you slightly into the ground, which is
				# detected here and shifts your y up slightly
				print(lerped_point, global_position)
				pp.position = lerped_point * 3 + global_position
				pp.position.y -= .5
				print(pp.position)
				if not len(get_world_2d().direct_space_state.intersect_point(pp, 1)) > 0:
					print("eyo")
					position.y -= .5
...
func animation_loop(delta) -> void:
	...
	if Util.any_input(["down", "up", "right", "left"]) and do_anim_dir and not is_stuck:
		do_anim_dir = false
		anim_dir = !anim_dir
	elif (is_stuck or is_on_ladder) and Input.is_action_just_pressed("jump"):
		anim_dir = !anim_dir
...
func jump_loop() -> void:
	...
	if Input.is_action_just_pressed("jump") and not is_dead and not is_knockedback and (is_on_floor() or is_on_ladder or has_extra_jump or can_swim or is_stuck):
...
func stuck_loop() -> void:
	...
	pp.position = global_position # + Vector2(0,-50)
	is_in_ground = len(get_world_2d().direct_space_state.intersect_point(pp, 1)) > 0 and bigmode
	is_stuck = is_overlapping and get_real_velocity().y == 0 and not is_in_ground and not is_on_floor() and not is_knockedback and not is_dead and bigmode
