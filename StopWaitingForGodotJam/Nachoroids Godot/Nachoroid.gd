...
func splode():
	...
	if can_split and !sploded:
		...
		for i in split_positions:
			...
			nacho.connect("receive_points", Callable(get_parent(), "UpdateScore"))
			...
			var random_dir = randf_range(0, 2 * PI)
			nacho.velocity += Vector2(randf_range(200.0, 300.0), 0.0).rotated(random_dir)
	else:
