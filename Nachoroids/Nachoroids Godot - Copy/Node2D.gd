...
func check_score(current_score):
	for element in highscore_list:
		if current_score["Score"] > element["Score"]:
			highscore_list.insert(highscore_list.find_last(element), current_score) #INSERT OVER ELEMENT
			current_score["Current"] = false
			break
		elif current_score["Score"] == element["Score"]:
			highscore_list.insert(highscore_list.find_last(element), current_score) #replace score - INSERT UNDER ELEMENT
			current_score["Current"] = false
			break
