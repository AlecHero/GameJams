...
func _process(_delta):
	...
	if menu_start.is_hovered():
		menu_start.add_font_override("font", sans_bold)
	else:
		menu_start.add_font_override("font", sans)

	if menu_highscore.is_hovered():
		menu_highscore.add_font_override("font", sans_bold)
	else:
		menu_highscore.add_font_override("font", sans)

func _on_CursorHue_mouse_entered():
	menu_slider_label.add_font_override("font", sans_bold)
func _on_CursorHue_mouse_exited():
	menu_slider_label.add_font_override("font", sans)

func _on_Start_pressed():
	MusicController.music_action("transition1")
	get_tree().change_scene("Stage.tscn")

func _on_Highscore_pressed():
	get_tree().change_scene("Highscore.tscn")
