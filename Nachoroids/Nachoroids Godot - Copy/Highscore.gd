...
func fix_score():
	...
	for element in highscore_list:
		if element["Current"]: #make the current score visible (if it made it)
			...
			rank_list[0].bbcode_text += boldify(rank_array.pop_front(), new_best) + "\n"
			rank_list[1].bbcode_text += boldify(str(element["Score"]), new_best) + "\n"
			rank_list[2].bbcode_text += boldify(str(element["Time"]), new_best) + "\n"
			rank_list[3].bbcode_text += boldify(str(element["Name"]), new_best) + "\n"
			element["Current"] = false
			General.save()
		else: #add the rest of the scores
			rank_list[0].bbcode_text += rank_array.pop_front() + "\n"
			rank_list[1].bbcode_text += str(element["Score"]) + "\n"
			rank_list[2].bbcode_text += str(element["Time"]) + "\n"
			rank_list[3].bbcode_text += str(element["Name"]) + "\n"
