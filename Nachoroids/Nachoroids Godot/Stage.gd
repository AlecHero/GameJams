...
func _process(_delta):
	$SadLayer/Overheat/ColorRect.material.set_shader_parameter("opacity", $HUD.heat_progress)
...
func _on_SpawnTimer_timeout():
	var nacho = Nachoroid.instantiate()
	add_child(nacho)
	nacho.connect("receive_points", Callable(self, "UpdateScore"))
	...
	nacho.rotation = mob_spawn_location.rotation + PI / 2 #Add randf_range(-PI / 4, PI / 4) for randomness
	nacho.velocity += Vector2(randf_range(SPEED_MIN, SPEED_MAX), 0.0).rotated(nacho.rotation)
